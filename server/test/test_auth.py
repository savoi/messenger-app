...
def test_register_user_success(client):
    ...
    response = client.post('/register', data=json.dumps(new_user), mimetype='application/json')
...
def test_register_email_already_exists(client):
    ...
    response1 = client.post('/register', data=json.dumps(new_user), mimetype='application/json')
    response2 = client.post('/register', data=json.dumps(new_user_2), mimetype='application/json')
...
def test_register_missing_email(client):
    ...
    assert response.json['error']['validation_error'] == "ValidationError (User:None) (Invalid email address: : ['email'])"
...
def test_register_username_too_short(client):
    ...
    response = client.post('/register', data=json.dumps(new_user), mimetype='application/json')
    assert response.status_code == 422
    assert response.json['error']['username'] == "Your username must be at least 2 characters."
...
def test_login_success(client):
    ...
    response = client.post('/register', data=json.dumps(non_existant_user), mimetype='application/json')
    response = client.post('/login', data=json.dumps(non_existant_user), mimetype='application/json')
    assert response.status_code == 201
    assert response.json['success'] == "User login successful!"
    cookies = response.headers.get_all('Set-Cookie')
    assert any('access_token_cookie' in cookie for cookie in cookies)
    assert any('csrf_access_token' in cookie for cookie in cookies)
